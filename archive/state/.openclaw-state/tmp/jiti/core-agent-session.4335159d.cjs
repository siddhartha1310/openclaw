...
var _slashCommands = require("./slash-commands.js");
...
class AgentSession {
  ...
  _bindExtensionCore(runner) {
    ...
    const reservedBuiltins = new Set(_slashCommands.BUILTIN_SLASH_COMMANDS.map((command) => command.name));
    const getCommands = () => {
  ...
  async switchSession(sessionPath) {
    ...
    const defaultThinkingLevel = this.settingsManager.getDefaultThinkingLevel() ?? _defaults.DEFAULT_THINKING_LEVEL;
    if (hasThinkingEntry) {
...
}exports.AgentSession = AgentSession; /* v9-85bd3c6b58c98cc1 */
