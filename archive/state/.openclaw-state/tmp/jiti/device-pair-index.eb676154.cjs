...
var _pluginSdk = require("openclaw/plugin-sdk");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
async function resolveTailnetHost(api) {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "status" || action === "pending") {
        const list = await (0, _pluginSdk.listDevicePairing)();
      ...
      if (action === "approve") {
        ...
        const approved = await (0, _pluginSdk.approveDevicePairing)(pending.requestId);
        if (!approved) {
          return { text: "Pairing request not found." };
        }
