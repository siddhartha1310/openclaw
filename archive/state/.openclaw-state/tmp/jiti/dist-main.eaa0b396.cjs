...
function buildSessionOptions(parsed, scopedModels, sessionManager, modelRegistry, settingsManager) {
  ...
  if (parsed.model) {
    ...
    if (resolved.warning) {
      console.warn(_chalk.default.yellow(`Warning: ${resolved.warning}`));
    }
    if (resolved.error) {
...
async function main(args) {
  ...
  if (parsed.apiKey) {
    if (!sessionOptions.model) {
      console.error(_chalk.default.red("--api-key requires a model to be specified via --model, --provider/--model, or --models"));
      process.exit(1);
    }
