"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = listChatCommandsForConfig;exports.c = normalizeCommandBody;exports.d = resolveCommandArgMenu;exports.f = serializeCommandArgs;exports.i = listChatCommands;exports.l = parseCommandArgs;exports.n = void 0;exports.o = listNativeCommandSpecs;exports.p = shouldHandleTextCommands;exports.r = findCommandByNativeName;exports.s = listNativeCommandSpecsForConfig;exports.t = buildCommandTextFromArgs;exports.u = resolveCommandArgChoices;var _rolldownRuntimeCbj13DAv = require("./rolldown-runtime-Cbj13DAv.js");
...
function normalizeArgValue(value) {
  ...
  let text;
  if (typeof value === "string") text = value.trim();else
  if (typeof value === "number" || typeof value === "boolean" || typeof value === "bigint") text = String(value).trim();else
  if (typeof value === "symbol") text = value.toString().trim();else
  if (typeof value === "function") text = value.toString().trim();else
...
function buildChatCommands() {
  ...
  defineChatCommand({
    ...
    args: [{
      ...
      type: "string",
      choices: ({ provider, model }) => (0, _thinkingBNkiJiB.r)(provider, model)
    }],
    argsMenu: "auto"
  }),
  defineChatCommand({
  ...
  ...(0, _thinkingBNkiJiB.f)().filter((dock) => dock.capabilities.nativeCommands).map((dock) => defineDockCommand(dock))];
...
function getNativeCommandSurfaces() {
  ...
  cachedNativeCommandSurfaces = new Set((0, _thinkingBNkiJiB.f)().filter((dock) => dock.capabilities.nativeCommands).map((dock) => dock.id));
...
var commands_registry_exports = exports.n = /* @__PURE__ */(0, _rolldownRuntimeCbj13DAv.t)({
...
function resolveDefaultCommandContext(cfg) {
  const resolved = (0, _modelSelectionCfNkGvWD.l)({
    cfg: cfg ?? {},
    defaultProvider: _modelSelectionCfNkGvWD.V,
    defaultModel: _modelSelectionCfNkGvWD.B
  });
  return {
    provider: resolved.provider ?? _modelSelectionCfNkGvWD.V,
    model: resolved.model ?? _modelSelectionCfNkGvWD.B
  };
}
...
function getCommandDetection(_cfg) {
  ...
  for (const cmd of commands) for (const alias of cmd.textAliases) {
    ...
    const escaped = (0, _registryDWvId1YW.x)(normalized);
    if (!escaped) continue;
