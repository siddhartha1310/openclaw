"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = logError;exports.c = createSubsystemLogger;exports.d = void 0;exports.f = clearActiveProgressLine;exports.i = logDebug;exports.l = stripAnsi;exports.m = unregisterActiveProgressLine;exports.n = runExec;exports.o = logInfo;exports.p = registerActiveProgressLine;exports.r = spawnWithFallback;exports.s = logWarn;exports.t = runCommandWithTimeout;exports.u = createNonExitingRuntime;var _registryDWvId1YW = require("./registry-DWvId1YW.js");
...
const defaultRuntime = exports.d = {
  log: (...args) => {
...
const requireConfig = (0, _nodeModule.createRequire)("file:///D:/workspace/appDev/openclaw/dist/dist/plugin-sdk/exec-eUUDM93d.js");
...
function resolveConsoleSettings() {
  let cfg = _registryDWvId1YW.nt.overrideSettings ?? (0, _registryDWvId1YW.at)();
  if (!cfg) if (_registryDWvId1YW.nt.resolvingConsoleSettings) cfg = void 0;else
  {
    _registryDWvId1YW.nt.resolvingConsoleSettings = true;
    try {
      cfg = loadConfigFallback();
    } finally {
      _registryDWvId1YW.nt.resolvingConsoleSettings = false;
    }
  }
  return {
...
function getConsoleSettings() {
  ...
  const cached = _registryDWvId1YW.nt.cachedConsoleSettings;
  if (!cached || consoleSettingsChanged(cached, settings)) _registryDWvId1YW.nt.cachedConsoleSettings = settings;
  return _registryDWvId1YW.nt.cachedConsoleSettings;
}
function shouldLogSubsystemToConsole(subsystem) {
  const filter = _registryDWvId1YW.nt.consoleSubsystemFilter;
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _registryDWvId1YW.rt)(level) <= (0, _registryDWvId1YW.rt)(settings.level);
}
...
function getColorForConsole() {
  ...
  return Boolean(process.stdout.isTTY || process.stderr.isTTY) || isRichConsoleEnv() ? new _chalk.Chalk({ level: 1 }) : new _chalk.Chalk({ level: 0 });
}
...
const CHANNEL_SUBSYSTEM_PREFIXES = new Set(_registryDWvId1YW.n);
...
function formatConsoleLine(opts) {
  ...
  return `${[(() => {
    ...
    if (_registryDWvId1YW.nt.consoleTimestampPrefix) return color.gray((/* @__PURE__ */new Date()).toISOString());
    return "";
...
function writeConsoleLine(level, line) {
  ...
  const sink = _registryDWvId1YW.nt.rawConsole ?? console;
  if (_registryDWvId1YW.nt.forceConsoleToStderr || level === "error" || level === "fatal") (sink.error ?? console.error)(sanitized);else
...
function createSubsystemLogger(subsystem) {
  let fileLogger = null;
  const getFileLogger = () => {
    if (!fileLogger) fileLogger = (0, _registryDWvId1YW.Z)({ subsystem });
    return fileLogger;
  };
  const emit = (level, message, meta) => {
    ...
    if (!(0, _registryDWvId1YW.V)() && subsystem === "agent/embedded" && /(sessionId|runId)=probe-/.test(consoleMessage)) return;
  ...
  const isFileEnabled = (level) => (0, _registryDWvId1YW.$)(level);
  return {
    subsystem,
    ...
    raw: (message) => {
      ...
      if (shouldLogSubsystemToConsole(subsystem)) {
        if (!(0, _registryDWvId1YW.V)() && subsystem === "agent/embedded" && /(sessionId|runId)=probe-/.test(message)) return;
...
function logInfo(message, runtime = defaultRuntime) {
  ...
  runtime.log((0, _registryDWvId1YW.B)(message));
  (0, _registryDWvId1YW.Q)().info(message);
}
function logWarn(message, runtime = defaultRuntime) {
  ...
  runtime.log((0, _registryDWvId1YW.q)(message));
  (0, _registryDWvId1YW.Q)().warn(message);
}
function logError(message, runtime = defaultRuntime) {
  ...
  runtime.error((0, _registryDWvId1YW.z)(message));
  (0, _registryDWvId1YW.Q)().error(message);
}
function logDebug(message) {
  (0, _registryDWvId1YW.Q)().debug(message);
  (0, _registryDWvId1YW.U)(message);
}
...
function resolveCommand(command) {
  ...
  if (_nodePath.default.extname(basename)) return command;
  if ([
  "npm",
  "pnpm",
  "yarn",
...
async function runExec(command, args, opts = 1e4) {
  ...
  try {
    ...
    if ((0, _registryDWvId1YW.G)()) logError((0, _registryDWvId1YW.z)(`Command failed: ${command} ${args.join(" ")}`));
    throw err;
  }
}
async function runCommandWithTimeout(argv, optionsOrTimeout) {
  ...
  const child = (0, _nodeChild_process.spawn)(resolvedCommand, argv.slice(1), {
    stdio,
    cwd,
    env: resolvedEnv,
