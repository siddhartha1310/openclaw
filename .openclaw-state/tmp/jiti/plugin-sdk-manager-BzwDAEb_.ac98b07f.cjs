"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = retryAsync;exports.n = resolveMemorySearchConfig;exports.r = resolveRetryConfig;exports.t = void 0;var _rolldownRuntimeCbj13DAv = require("./rolldown-runtime-Cbj13DAv.js");
...
var _sqliteCknMfW4r = require("./sqlite-CknMfW4r.js");
...
var _chokidar = _interopRequireDefault(require("chokidar"));function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
async function retryAsync(fn, attemptsOrOptions = 3, initialDelayMs = 300) {
  if (typeof attemptsOrOptions === "number") {
    ...
    for (let i = 0; i < attempts; i += 1) try {
      ...
      await (0, _registryDWvId1YW.F)(initialDelayMs * 2 ** i);
    }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) try {
    ...
    await (0, _registryDWvId1YW.F)(delay);
  }
  throw lastErr ?? /* @__PURE__ */new Error("Retry failed");
}
...
function resolveStorePath(agentId, raw) {
  const stateDir = (0, _pathsZQWYGl2V.s)(process.env, _nodeOs.default.homedir);
  const fallback = _nodePath.default.join(stateDir, "memory", `${agentId}.sqlite`);
  if (!raw) return fallback;
  return (0, _registryDWvId1YW.j)(raw.includes("{agentId}") ? raw.replaceAll("{agentId}", agentId) : raw);
}
function mergeConfig(defaults, overrides, agentId) {
  ...
  const overlap = (0, _registryDWvId1YW.y)(chunking.overlap, 0, Math.max(0, chunking.tokens - 1));
  const minScore = (0, _registryDWvId1YW.y)(query.minScore, 0, 1);
  const vectorWeight = (0, _registryDWvId1YW.y)(hybrid.vectorWeight, 0, 1);
  const textWeight = (0, _registryDWvId1YW.y)(hybrid.textWeight, 0, 1);
  const sum = vectorWeight + textWeight;
  const normalizedVectorWeight = sum > 0 ? vectorWeight / sum : DEFAULT_HYBRID_VECTOR_WEIGHT;
  const normalizedTextWeight = sum > 0 ? textWeight / sum : DEFAULT_HYBRID_TEXT_WEIGHT;
  const candidateMultiplier = (0, _registryDWvId1YW.v)(hybrid.candidateMultiplier, 1, 20);
  const deltaBytes = (0, _registryDWvId1YW.v)(sync.sessions.deltaBytes, 0, Number.MAX_SAFE_INTEGER);
  const deltaMessages = (0, _registryDWvId1YW.v)(sync.sessions.deltaMessages, 0, Number.MAX_SAFE_INTEGER);
  return {
    enabled,
    sources,
    extraPaths,
...
function resolveMemorySearchConfig(cfg, agentId) {
  ...
  const overrides = (0, _agentScopeBCNbpzc.n)(cfg, agentId)?.memorySearch;
...
const debugEmbeddings$1 = (0, _env7LNI1cfd.t)(process.env.OPENCLAW_DEBUG_MEMORY_EMBEDDINGS);
const log$4 = (0, _execEUUDM93d.c)("memory/embeddings");
...
async function resolveGeminiEmbeddingClient(options) {
  ...
  const apiKey = remoteApiKey ? remoteApiKey : (0, _modelSelectionCfNkGvWD.S)(await (0, _modelSelectionCfNkGvWD.C)({
    provider: "google",
    cfg: options.config,
...
async function importNodeLlamaCpp() {
  return Promise.resolve().then(() => jitiImport("node-llama-cpp").then((m) => _interopRequireWildcard(m)));
}
...
function canAutoSelectLocal(options) {
  ...
  const resolved = (0, _registryDWvId1YW.j)(modelPath);
  try {
...
function isMissingApiKeyError(err) {
  return (0, _errorsDsAYQPj.n)(err).includes("No API key found for provider");
}
...
async function createEmbeddingProvider$1(options) {
  ...
  const formatPrimaryError = (err, provider) => provider === "local" ? formatLocalSetupError(err) : (0, _errorsDsAYQPj.n)(err);
  ...
  try {
    ...
    if (fallback && fallback !== "none" && fallback !== requestedProvider) try {
      ...
      throw new Error(`${reason}\n\nFallback to ${fallback} failed: ${(0, _errorsDsAYQPj.n)(fallbackErr)}`, { cause: fallbackErr });
    }
...
function formatLocalSetupError(err) {
  const detail = (0, _errorsDsAYQPj.n)(err);
...
const debugEmbeddings = (0, _env7LNI1cfd.t)(process.env.OPENCLAW_DEBUG_MEMORY_EMBEDDINGS);
const log$3 = (0, _execEUUDM93d.c)("memory/embeddings");
...
function buildGeminiUploadBody(params) {
  const boundary = `openclaw-${(0, _sqliteCknMfW4r.l)(params.displayName)}`;
...
async function submitGeminiBatch(params) {
  ...
  const uploadPayload = buildGeminiUploadBody({
    ...
    displayName: `memory-embeddings-${(0, _sqliteCknMfW4r.l)(String(Date.now()))}`
  });
...
async function runGeminiEmbeddingBatches(params) {
  ...
  await (0, _sqliteCknMfW4r.h)(tasks, params.concurrency);
  return byCustomId;
}
...
async function submitOpenAiBatch(params) {
  ...
  form.append("file", new Blob([jsonl], { type: "application/jsonl" }), `memory-embeddings.${(0, _sqliteCknMfW4r.l)(String(Date.now()))}.jsonl`);
...
async function runVoyageEmbeddingBatches(params) {
  ...
  const tasks = groups.map((group, groupIndex) => async () => {
    ...
    const remaining = new Set(group.map((request) => request.custom_id));
    if (contentRes.body) {
      const reader = (0, _nodeReadline.createInterface)({
        input: _nodeStream.Readable.fromWeb(contentRes.body),
        terminal: false
      });
...
const log$2 = (0, _execEUUDM93d.c)("memory");
var MemoryManagerEmbeddingOps = class {
  ...
  loadEmbeddingCache(hashes) {
    ...
    for (let start = 0; start < unique.length; start += batchSize) {
      ...
      for (const row of rows) out.set(row.hash, (0, _sqliteCknMfW4r.p)(row.embedding));
    }
    return out;
  }
  ...
  computeProviderKey() {
    if (this.provider.id === "openai" && this.openAi) {
      ...
      return (0, _sqliteCknMfW4r.l)(JSON.stringify({
        provider: "openai",
  ...
  collectCachedEmbeddings(chunks) {
    ...
    for (let i = 0; i < chunks.length; i += 1) {
      ...
      if (hit && hit.length > 0) embeddings[i] = hit;else
  ...
  buildBatchCustomId(params) {
    return (0, _sqliteCknMfW4r.l)(`${params.source}:${params.entry.path}:${params.chunk.startLine}:${params.chunk.endLine}:${params.chunk.hash}:${params.index}`);
  }
  ...
  async indexFile(entry, options) {
    const content = options.content ?? (await _promises.default.readFile(entry.absPath, "utf-8"));
    const chunks = enforceEmbeddingMaxInputTokens(this.provider, (0, _sqliteCknMfW4r.o)(content, this.settings.chunking).filter((chunk) => chunk.text.trim().length > 0));
    if (options.source === "sessions" && "lineMap" in entry) (0, _sqliteCknMfW4r.m)(chunks, entry.lineMap);
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const id = (0, _sqliteCknMfW4r.l)(`${options.source}:${entry.path}:${chunk.startLine}:${chunk.endLine}:${chunk.hash}:${this.provider.model}`);
...
async function searchVector(params) {
  ...
  FROM ${params.vectorTable} v\n  JOIN chunks c ON c.id = v.id\n WHERE c.model = ?${params.sourceFilterVec.sql}\n ORDER BY dist ASC\n LIMIT ?`).all(vectorToBlob(params.queryVec), params.providerModel, ...params.sourceFilterVec.params, params.limit).map((row) => ({
    ...
    score: 1 - row.dist,
    snippet: (0, _registryDWvId1YW.R)(row.text, params.snippetMaxChars),
    source: row.source
  }));
  return listChunks({
    db: params.db,
    ...
    chunk,
    score: (0, _sqliteCknMfW4r.s)(params.queryVec, chunk.embedding)
    ...
    score: entry.score,
    snippet: (0, _registryDWvId1YW.R)(entry.chunk.text, params.snippetMaxChars),
...
function listChunks(params) {
 ...
 WHERE model = ?${params.sourceFilter.sql}`).all(params.providerModel, ...params.sourceFilter.params).map((row) => ({
    ...
    endLine: row.end_line,
    text: row.text,
    embedding: (0, _sqliteCknMfW4r.p)(row.embedding),
    source: row.source
  }));
}
...
async function loadSqliteVecExtension(params) {
  try {
    const sqliteVec = await Promise.resolve().then(() => jitiImport("sqlite-vec").then((m) => _interopRequireWildcard(m)));
...
const log$1 = (0, _execEUUDM93d.c)("memory");
function shouldIgnoreMemoryWatchPath(watchPath) {
  return _nodePath.default.normalize(watchPath).split(_nodePath.default.sep).map((segment) => segment.trim().toLowerCase()).some((segment) => IGNORED_MEMORY_WATCH_DIR_NAMES.has(segment));
}
var MemoryManagerSyncOps = class {
  ...
  async loadVectorExtension() {
    ...
    try {
      const resolvedPath = this.vector.extensionPath?.trim() ? (0, _registryDWvId1YW.j)(this.vector.extensionPath) : void 0;
  ...
  openDatabase() {
    const dbPath = (0, _registryDWvId1YW.j)(this.settings.store.path);
    return this.openDatabaseAtPath(dbPath);
  }
  openDatabaseAtPath(dbPath) {
    (0, _sqliteCknMfW4r.c)(_nodePath.default.dirname(dbPath));
    const { DatabaseSync } = (0, _sqliteCknMfW4r.t)();
  ...
  ensureWatcher() {
    ...
    _nodePath.default.join(this.workspaceDir, "MEMORY.md"),
    _nodePath.default.join(this.workspaceDir, "memory.md"),
    _nodePath.default.join(this.workspaceDir, "memory", "**", "*.md")]
    );
    const additionalPaths = (0, _sqliteCknMfW4r.f)(this.workspaceDir, this.settings.extraPaths);
    for (const entry of additionalPaths) try {
      const stat = _nodeFs.default.lstatSync(entry);
      if (stat.isSymbolicLink()) continue;
      if (stat.isDirectory()) {
        watchPaths.add(_nodePath.default.join(entry, "**", "*.md"));
        continue;
      }
      if (stat.isFile() && entry.toLowerCase().endsWith(".md")) watchPaths.add(entry);
    } catch {}
    this.watcher = _chokidar.default.watch(Array.from(watchPaths), {
      ignoreInitial: true,
  ...
  ensureSessionListener() {
    ...
    this.sessionUnsubscribe = (0, _transcriptEventsDf8wfs2b.n)((update) => {
  ...
  async countNewlines(absPath, start, end) {
    ...
    const handle = await _promises.default.open(absPath, "r");
    try {
      let offset = start;
      let count = 0;
  ...
  isSessionFileForAgent(sessionFile) {
    ...
    const sessionsDir = (0, _pathsDxk5i2ju.o)(this.agentId);
  ...
  async syncMemoryFiles(params) {
    const files = await (0, _sqliteCknMfW4r.d)(this.workspaceDir, this.settings.extraPaths);
    const fileEntries = await Promise.all(files.map(async (file) => (0, _sqliteCknMfW4r.a)(file, this.workspaceDir)));
    ...
    await (0, _sqliteCknMfW4r.h)(fileEntries.map((entry) => async () => {
  ...
  async syncSessionFiles(params) {
    const files = await (0, _sqliteCknMfW4r.r)(this.agentId);
    const activePaths = new Set(files.map((file) => (0, _sqliteCknMfW4r.i)(file)));
    ...
    await (0, _sqliteCknMfW4r.h)(files.map((absPath) => async () => {
      ...
      const entry = await (0, _sqliteCknMfW4r.n)(absPath);
      if (!entry) {
        if (params.progress) {
          params.progress.completed += 1;
          params.progress.report({
  ...
  async runSafeReindex(params) {
    ...
    const restoreOriginalState = () => {
      if (originalDbClosed) this.db = this.openDatabaseAtPath(dbPath);else
      this.db = originalDb;
...
var manager_exports = exports.t = /* @__PURE__ */(0, _rolldownRuntimeCbj13DAv.t)({ MemoryIndexManager: () => MemoryIndexManager });
...
var MemoryIndexManager = class MemoryIndexManager {
  static async get(params) {
    ...
    const manager = new MemoryIndexManager({
      ...
      providerResult: await createEmbeddingProvider$1({
        config: cfg,
        agentDir: (0, _agentScopeBCNbpzc.r)(cfg, agentId),
  ...
  async readFile(params) {
    ...
    const absPath = _nodePath.default.isAbsolute(rawPath) ? _nodePath.default.resolve(rawPath) : _nodePath.default.resolve(this.workspaceDir, rawPath);
    const relPath = _nodePath.default.relative(this.workspaceDir, absPath).replace(/\\/g, "/");
    const allowedWorkspace = relPath.length > 0 && !relPath.startsWith("..") && !_nodePath.default.isAbsolute(relPath) && (0, _sqliteCknMfW4r.u)(relPath);
