"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = stripThinkingTagsFromText;exports.a = decodeDataUrl;exports.b = ensureOpenClawModelsJson;exports.c = extractAssistantThinking;exports.d = formatReasoningMessage;exports.f = inferToolMetaFromArgs;exports.g = stripMinimaxToolCallXml;exports.h = stripDowngradedToolCallText;exports.i = coerceImageModelConfig;exports.l = extractThinkingFromTaggedStream;exports.m = promoteThinkingTagsToBlocks;exports.n = void 0;exports.o = resolveProviderVisionModelFromConfig;exports.p = isAssistantMessage;exports.r = coerceImageAssistantText;exports.s = extractAssistantText;exports.t = describeImageWithModel;exports.u = extractThinkingFromTaggedText;exports.v = resolveToolDisplay;exports.x = minimaxUnderstandImage;exports.y = stripReasoningTagsFromText;var _rolldownRuntimeCbj13DAv = require("./rolldown-runtime-Cbj13DAv.js");
...
var _piModelDiscoveryDtR631Ph = require("./pi-model-discovery-DtR631Ph.js");
var _nodePath = _interopRequireDefault(require("node:path"));
var _promises = _interopRequireDefault(require("node:fs/promises"));
var _piAi = require("@mariozechner/pi-ai");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
async function minimaxUnderstandImage(params) {
  const apiKey = (0, _modelSelectionCfNkGvWD.L)(params.apiKey);
  ...
  if (!(0, _registryDWvId1YW.w)(json)) {
    const trace = traceId ? ` Trace-Id: ${traceId}` : "";
    throw new Error(`MiniMax VLM response was not JSON.${trace}`);
  }
  const baseResp = (0, _registryDWvId1YW.w)(json.base_resp) ? json.base_resp : {};
...
async function ensureOpenClawModelsJson(config, agentDirOverride) {
  const cfg = config ?? (0, _configLDeTe_Qk.n)();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : (0, _modelSelectionCfNkGvWD.I)();
  const explicitProviders = cfg.models?.providers ?? {};
  const providers = mergeProviders({
    implicit: await (0, _modelSelectionCfNkGvWD._)({
      agentDir,
      explicitProviders
    }),
    explicit: explicitProviders
  });
  const implicitBedrock = await (0, _modelSelectionCfNkGvWD.h)({
    agentDir,
    config: cfg
  });
  if (implicitBedrock) {
  ...
  const implicitCopilot = await (0, _modelSelectionCfNkGvWD.g)({ agentDir });
  ...
  if (mode === "merge") {
    ...
    if ((0, _registryDWvId1YW.w)(existing) && (0, _registryDWvId1YW.w)(existing.providers)) mergedProviders = {
      ...existing.providers,
      ...providers
    };
  }
  const normalizedProviders = (0, _modelSelectionCfNkGvWD.m)({
  ...
  await _promises.default.writeFile(targetPath, next, { mode: 384 });
  return {
    agentDir,
    wrote: true
  };
...
function resolveToolDisplay(params) {
  ...
  if (detail) detail = (0, _registryDWvId1YW.N)(detail);
  return {
    name,
    emoji,
    title,
...
function formatToolDetail(display) {
  ...
  if (display.detail) parts.push((0, _redactDLALByE.n)(display.detail));
...
function extractAssistantText(msg) {
  ...
  return (0, _piEmbeddedHelpersCAhnRvqw.N)((Array.isArray(msg.content) ? msg.content.filter(isTextBlock).map((c) => stripThinkingTagsFromText(stripDowngradedToolCallText(stripMinimaxToolCallXml(c.text))).trim()).filter(Boolean) : []).join("\n").trim(), { errorContext: msg.stopReason === "error" || Boolean(msg.errorMessage?.trim()) });
}
...
var image_exports = exports.n = /* @__PURE__ */(0, _rolldownRuntimeCbj13DAv.t)({ describeImageWithModel: () => describeImageWithModel });
async function describeImageWithModel(params) {
  await ensureOpenClawModelsJson(params.cfg, params.agentDir);
  const authStorage = (0, _piModelDiscoveryDtR631Ph.t)(params.agentDir);
  const model = (0, _piModelDiscoveryDtR631Ph.n)(authStorage, params.agentDir).find(params.provider, params.model);
  ...
  const apiKey = (0, _modelSelectionCfNkGvWD.S)(await (0, _modelSelectionCfNkGvWD.b)({
    model,
    cfg: params.cfg,
  ...
  return {
    text: coerceImageAssistantText({
      message: await (0, _piAi.complete)(model, { messages: [{
          role: "user",
          content: [{
            type: "text",
